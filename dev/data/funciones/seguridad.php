...
function limpiarURL($text){
 ...
 function limpiarContenido($content) {

	$content = preg_replace('#<table.*?>(.*?)</table>#i', '<table>\1</table>', $content);
	$content = preg_replace('#<tr.*?>(.*?)</tr>#i', '<tr>\1</tr>', $content);
	$content = preg_replace('#<td.*?>(.*?)</td>#i', '<td>\1</td>', $content);
	$content = preg_replace('#<div.*?>(.*?)</p>#i', '<div>\1</div>', $content);
	...
	$content = preg_replace('#font.*?>(.*?)</font>#i', '<font>\1</font>', $content);
